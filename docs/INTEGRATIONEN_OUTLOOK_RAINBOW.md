...
Die Administratorseite **Mein Bereich → Systemeinstellungen** speichert nur nicht geheime Angaben. Beide Anbindungen sind bis zur Freigabe, Implementierung und Prüfung durch die Stadt-IT **inaktiv**. Das Speichern aktiviert weder Synchronisation noch Telefonie.

## Outlook-Kontakte

- Administrator wählt eine Quelle: Kontakte eines dienstlichen Postfachs oder Organisationskontakte. Für das Postfach können Adresse und Kontaktordner-ID vorgemerkt werden.
...
- Administrator kann die angerufene Zentralnummer vormerken. `integration_rainbow_calls` ist ein geschützter, zunächst leerer Speicher für spätere eingehende Ereignisse mit Rainbow-Benutzer-ID, Anruf-ID, Rufnummern, Zustand und Zeitpunkt. Die Kombination aus Benutzer-ID und Anruf-ID entdoppelt Ereignisse.
