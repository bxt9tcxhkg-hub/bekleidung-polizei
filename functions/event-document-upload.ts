...
export const onRequestPost: PagesFunction<Env> = async (context) => {
  ...
  const allowed = await Promise.all([
    canManageZentrale(context.request, context.env),
    isZentralistOnDuty(context.request, context.env),
  ])
  if (!allowed.some(Boolean)) return new Response('Forbidden', { status: 403 })

  const eventId = context.request.headers.get('X-Event-Id') ?? ''
  if (!UUID.test(eventId)) {
    return new Response(JSON.stringify({ error: 'Ungültiges Ereignis.' }), { status: 400 })
  }
  ...
  if (contentLength === null || contentLength > 20_000_000) {
  ...
  if (!ALLOWED_TYPES.has(contentType) && !contentType.startsWith('image/')) {
    return new Response(JSON.stringify({ error: 'Nur PDF oder Bild zulässig.' }), { status: 400 })
  }

  const extension = (fileName.split('.').pop() ?? 'bin').replace(/[^A-Za-z0-9]/g, '').slice(0, 10)
  const key = `ereignis-dokumente/${eventId}/${crypto.randomUUID()}.${extension}`
