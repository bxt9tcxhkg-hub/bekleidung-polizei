-- Baustellen-Markierungen auf der Zentrale-Karte - unabhängig vom (eher
-- formellen) Straßenzustandsbericht. Manuell gepflegter Streckenabschnitt
-- (Start-/Endpunkt, aktuell eine gerade Linie) statt nur ein Punkt. Jeder
-- mit Zentrale-Zugriff kann eine Wahrnehmung melden (z. B. im Außendienst),
-- die dann als "gemeldet" (unbestätigt) auf der Karte erscheint, bis ein
-- Sachbearbeiter/Genehmiger sie prüft und bestätigt oder direkt selbst
-- bestätigt anlegt.
...
create policy "Baustellen melden" on public.zentrale_baustellen for insert
  with check (
    ...
    -- Wer nicht verwalten darf, kann nur als "gemeldet" (unbestätigt) anlegen -
    -- die Bestätigung ist Sachbearbeitern/Genehmigern vorbehalten.
