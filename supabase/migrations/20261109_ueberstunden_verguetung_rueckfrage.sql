-- Erweiterung der Überstundenmeldung um zwei vom Nutzer angeforderte Punkte:
--
-- 1. Vergütungsart (Auszahlung/Stundenersatz) - in der offiziellen Vorlage
--    als Radio-Auswahl unter "Vergütung:" geführt, Auszahlung als Vorgabe.
-- 2. Dritte Genehmiger-Entscheidung "Rückfrage" neben genehmigen/ablehnen -
--    schickt die Meldung zur Klärung an den/die Bedienstete/n zurück; von
--    dort aus editierbar und erneut einreichbar (wie ein Entwurf), im
--    Unterschied zu genehmigt/abgelehnt aber keine Endentscheidung.
...
BEGIN
  IF public.is_genehmiger() AND NEW.beamter_id <> (SELECT auth.uid()) THEN
    ...
    -- genehmigt/abgelehnt/zur Rückfrage zurückgelegt, die Angaben selbst
    -- bleiben unverändert.
    ...
    END IF;
  ELSE
    -- Eigene Meldung: frei editierbar als Entwurf oder nach einer
    -- Rückfrage, als eingereichte Meldung nur noch zurückziehbar (zurück zu
    -- Entwurf) - einmal genehmigt/abgelehnt ist sie unveränderlich.
    ...
    END IF;
    -- entwurf/eingereicht sind als Zielstatus immer erlaubt (Speichern/
    -- Einreichen/Zurückziehen); eine Meldung mit Rückfrage darf beim reinen
    -- Bearbeiten (ohne Statuswechsel) auch in diesem Status bleiben - nur
    -- der Genehmiger darf NEU auf Rückfrage setzen, nicht der/die Bedienstete.
