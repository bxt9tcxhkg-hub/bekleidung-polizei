...
--
-- 1. Ein unabgeschickter Entwurf oder eine zur Rückfrage zurückgelegte
--    Meldung zählte bisher mit zur 8-Std-Schwelle eines Sonn-/Feiertags,
--    obwohl noch nicht feststeht, ob/wann diese Stunden überhaupt
--    eingereicht werden. Jetzt zählen nur noch eingereichte, genehmigte
--    oder zur Rückfrage stehende Meldungen zum gemeinsamen Topf - ein
--    reiner Entwurf verbraucht keine Schwelle. Rückfrage zählt bewusst mit
--    (die Stunden wurden tatsächlich geleistet, nur eine Detailfrage ist
--    offen), Ablehnung weiterhin nicht.
-- 2. Die Monatsübersicht/Sammelansicht fasste Auszahlung und Stundenersatz
--    zu einer gemeinsamen Summe je Beamten zusammen - die Lohnberechnung
--    kann damit nicht unterscheiden, welche Stunden ausbezahlt und welche
--    als Zeitausgleich zu verbuchen sind. Jetzt eine eigene Zeile je
--    Beamten UND Vergütungsart.
-- 3. Die Dauer-Berechnung nutzte reine (zeitzonenlose) timestamp-Subtraktion
--    - an den zwei jährlichen Umstellungstagen (Ende März/Oktober) weicht
--    das von der tatsächlich vergangenen Zeit um eine Stunde ab. Die
--    Tagesgrenzen/Fenster (06-19 Uhr usw.) bleiben bewusst Wanduhrzeit-
--    basiert (das ist ja gerade gemeint), nur die tatsächliche Dauer
--    zwischen zwei Zeitpunkten wird jetzt über die Zeitzone Europe/Vienna
--    in echte verstrichene Sekunden umgerechnet.
...
BEGIN
  ...
  WHILE cur < bis LOOP
    ...
    -- Über Europe/Vienna in echte verstrichene Sekunden umrechnen (siehe
    -- Kommentar oben) - Tagesgrenzen selbst bleiben Wanduhrzeit-basiert.
...
BEGIN
  ...
  WHILE cur < p_bis LOOP
    ...
    IF public.ist_sonn_oder_feiertag(tagesbeginn::date) THEN
      ...
      -- Nur Meldungen, die tatsächlich zum gemeinsamen Topf zählen (siehe
      -- ueberstunden_zaehlt_zum_topf) - ein unabgeschickter Entwurf
      -- verbraucht keine Schwelle.
...
-- Reallokation/Neuberechnung betrifft jetzt Meldungen, die den Topf
-- verlassen ODER betreten - also auch eine Einreichung (entwurf/rueckfrage
-- -> eingereicht) oder ein Zurückziehen (eingereicht -> entwurf), nicht nur
-- eine Ablehnung.
