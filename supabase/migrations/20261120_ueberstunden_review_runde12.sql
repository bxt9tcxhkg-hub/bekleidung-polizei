-- Reaktion auf ein Review-Finding (P2): die bereits vorhandene DST-Korrektur
-- (AT TIME ZONE 'Europe/Vienna' für echte verstrichene Sekunden statt reiner
-- Wanduhr-Subtraktion) löst die Frühjahrs-Umstellung korrekt, lässt aber die
-- Herbst-Umstellung weiter falsch: die Stunde 02:00-03:00 kommt in der Nacht
-- der Rückstellung zweimal vor (Wanduhrzeit ist mehrdeutig), und Postgres löst
-- eine mehrdeutige lokale Zeit beim Konvertieren IMMER auf die SPÄTERE
-- (Normalzeit-)Variante auf. Ein durchgehender Dienst über genau diese
-- wiederholte Stunde (von "02:00" bis "03:00" an diesem einen Datum) sind
-- real ZWEI verstrichene Stunden, wurde bisher aber als eine gezählt.
--
-- ueberstunden_vienna_instant löst diese Mehrdeutigkeit gezielt auf: erkannt
-- wird sie daran, dass der um eine Stunde frühere Zeitpunkt, zurück nach Wien
-- konvertiert, exakt wieder dieselbe Wanduhrzeit ergibt (beide Zeitzonen-
-- Offsets bilden dann auf dieselbe lokale Schreibweise ab) - nur dann weicht
-- das Ergebnis überhaupt vom bisherigen Verhalten ab. Der Beginn eines
-- Abschnitts bekommt bei Mehrdeutigkeit die FRÜHERE (Sommerzeit-)Variante,
-- das Ende die SPÄTERE (Normalzeit-)Variante - ein Dienst, der buchstäblich
-- über die wiederholte Stunde hinweggeht, zählt damit die tatsächlich
-- geleisteten zwei Stunden (Zweifel zugunsten des Beamten, wie bei einer
-- Arbeitszeit-/Entgeltberechnung üblich), während jede eindeutige Zeit exakt
-- wie bisher behandelt wird.
