-- Überstundenmeldung: die Aufschlüsselung nach Lohnarten erfolgt jetzt
-- automatisch aus einem Zeitraum (von Datum/Uhrzeit bis Datum/Uhrzeit) statt
-- manueller Eingabe je Kategorie (siehe lib/ueberstunden.ts,
-- berechneAufschluesselung() + lib/austrianHolidays.ts für die
-- österreichischen Feiertage - bundesweit gültig, keine gesonderten
-- Vorarlberg-Feiertage). Dafür wird aus der bisherigen Ein-Tages-Annahme
-- (datum/zeit_von/zeit_bis) ein echter, auch mehrtägiger Zeitraum:
-- datum -> von_datum, zeit_von -> von_zeit, zeit_bis -> bis_zeit, neu dazu
-- bis_datum. Tabelle ist erst seit kurzem live und noch ohne echte Daten.
