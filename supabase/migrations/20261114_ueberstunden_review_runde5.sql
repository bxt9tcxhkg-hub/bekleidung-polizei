-- Reaktion auf ein weiteres Review-Finding:
--
-- Die 100-%-Zuteilung an einem Sonn-/Feiertag richtete sich bisher nach der
-- Reihenfolge, in der Meldungen ERFASST wurden (created_at) - trägt ein
-- Beamter zuerst die 14-20-Uhr-Meldung ein und danach die 06-12-Uhr-Meldung
-- desselben Tages, bekäme die spätere (aber zeitlich frühere) Meldung
-- fälschlich die 200-%-Kategorie. Die Priorität richtet sich jetzt nach der
-- tatsächlich gemeldeten Arbeitszeit (von_datum/von_zeit) - unabhängig
-- davon, in welcher Reihenfolge die Meldungen erfasst wurden. p_von ist
-- dafür bereits als Parameter vorhanden (der eigene Start-Zeitpunkt der
-- Meldung); id bleibt als deterministischer Tie-Break bei exakt gleichem
-- Start erhalten.
...
BEGIN
  ...
  WHILE cur < p_bis LOOP
    ...
    IF public.ist_sonn_oder_feiertag(tagesbeginn::date) THEN
      ...
      -- Priorität nach tatsächlicher Arbeitszeit (von), nicht nach
      -- Erfassungszeitpunkt - siehe Kommentar oben.
...
-- Reallokation folgt jetzt ebenfalls der Arbeitszeit-Priorität: betroffen
-- sind Meldungen desselben Beamten/Tages, deren Start ZEITLICH NACH der
-- geänderten Meldung liegt (nicht mehr "später erfasst").
...
-- p_created_at (timestamptz) -> p_von (timestamp ohne Zeitzone, siehe oben)
-- - andere Signatur, daher alte Funktion explizit entfernen.
DROP FUNCTION IF EXISTS public.ueberstunden_reallocate_nachfolgende(uuid, date, timestamptz, uuid);
REVOKE ALL ON FUNCTION public.ueberstunden_reallocate_nachfolgende(uuid, date, timestamp, uuid) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.ueberstunden_reallocate_nachfolgende(uuid, date, timestamp, uuid) TO authenticated;
