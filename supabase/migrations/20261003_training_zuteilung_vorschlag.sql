-- Einsatztraining: Sachbearbeiter schlägt vor, wer ein Modul absolvieren soll
-- (mit oder ohne konkreten Termin) – die Genehmiger-Rolle entscheidet über
-- die tatsächliche Einteilung. Betrifft auch Selbstanmeldungen: das bisherige
-- "Anmelden" zu einem Termin ist ab jetzt ebenfalls nur noch ein Vorschlag,
-- der erst nach Bestätigung durch den Genehmiger zu einer echten Anmeldung
-- (einsatz_training_registrations) wird.

-- Reine Genehmiger-Prüfung (ohne Bereichsrollen-Fallback) – die Entscheidung
-- über einen Trainingsvorschlag ist bewusst dem Genehmiger vorbehalten, nicht
-- dem allgemeinen Einsatzmittel-Sachbearbeiter.
...
-- Sachbearbeiter/Admin/Genehmiger können für jede Person vorschlagen (mit oder ohne Termin),
-- eine Person kann sich selbst nur zu einem konkret wählbaren, offenen Termin vorschlagen.
...
-- Zurückziehen eines noch offenen Vorschlags: wer ihn vorgeschlagen hat, oder die Verwaltung.
...
-- RPC für die Entscheidung: nur der Genehmiger darf einen offenen Vorschlag einteilen
-- (mit Termin, ggf. neu gewählt) oder ablehnen. Bei Einteilung entsteht atomar die
-- echte Anmeldung, damit Anwesenheit/Abschluss unverändert weiterlaufen.
