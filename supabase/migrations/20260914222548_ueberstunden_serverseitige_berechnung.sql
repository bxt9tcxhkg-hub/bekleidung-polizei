...
LANGUAGE sql IMMUTABLE AS $$
  SELECT tag IN (
    make_date(extract(year from tag)::int, 1, 1),
    make_date(extract(year from tag)::int, 1, 6),
    public.oster_sonntag(extract(year from tag)::int) + 1,
    make_date(extract(year from tag)::int, 5, 1),
    public.oster_sonntag(extract(year from tag)::int) + 39,
    public.oster_sonntag(extract(year from tag)::int) + 50,
    public.oster_sonntag(extract(year from tag)::int) + 60,
    make_date(extract(year from tag)::int, 8, 15),
    make_date(extract(year from tag)::int, 10, 26),
    make_date(extract(year from tag)::int, 11, 1),
    make_date(extract(year from tag)::int, 12, 8),
    make_date(extract(year from tag)::int, 12, 25),
    make_date(extract(year from tag)::int, 12, 26)
  );
$$;
...
RETURNS TABLE(tag date, stunden numeric) LANGUAGE plpgsql IMMUTABLE AS $$
DECLARE
  cur timestamp := von;
