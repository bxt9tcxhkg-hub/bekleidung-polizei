-- Reaktion auf drei weitere Review-Findings zur serverseitigen Berechnung
-- (20261108_ueberstunden_serverseitige_berechnung.sql):
--
-- 1. Race Condition: zwei gleichzeitig eingefügte Meldungen desselben
--    Beamten für denselben Sonn-/Feiertag konnten beide von 0 bereits
--    verwendeten Stunden ausgehen (jede Transaktion sieht die andere,
--    noch nicht committete Zeile nicht) und so die 8-Std-Schwelle jeweils
--    voll ausnutzen. Fix: ein Advisory Lock je (Beamter, Kalendertag)
--    serialisiert die Berechnung.
-- 2. Die "nur bei geändertem Zeitraum neu berechnen"-Optimierung im
--    Autofill-Trigger ließ bei einer reinen Inhalts-Änderung (z. B. nur der
--    Grund) die vom Client mitgeschickten std_*-Werte unverändert stehen -
--    also exakt die Lücke aus Finding 1 der vorherigen Runde, nur für den
--    Fall "Zeitraum bleibt gleich". Fix: bei JEDER Änderung wird neu
--    berechnet; die dadurch nötige Anpassung an enforce_ueberstunden_update
--    (unten) verhindert, dass das eine Genehmiger-Entscheidung blockiert.
-- 3. Wird eine Meldung, die an einem Sonn-/Feiertag einen Teil der
--    8-Std-Schwelle belegt hat, abgelehnt oder gelöscht, behielten andere
--    Meldungen desselben Tages ihre alte (nun falsche) Aufteilung. Fix:
--    Die Zuteilung erfolgt jetzt nach fester Reihenfolge (frühestes
--    created_at zuerst bekommt die 100-%-Stunden); nur SPÄTER erstellte
--    Meldungen können von einer früheren abhängen (nie umgekehrt), und ein
--    AFTER-Trigger stößt bei Statuswechsel/Löschung/Zeitraumänderung eine
--    Neuberechnung aller nachfolgenden Meldungen desselben Tages an.
...
BEGIN
  ...
  WHILE cur < p_bis LOOP
    ...
    IF public.ist_sonn_oder_feiertag(tagesbeginn::date) THEN
      -- Serialisiert gegen gleichzeitige Einfüge-/Änderungsversuche
      -- desselben Beamten/Tages (sonst könnten zwei Transaktionen dieselbe
      -- "0 bereits verwendet"-Momentaufnahme sehen). Wird am Ende der
      -- Transaktion automatisch freigegeben.
      PERFORM pg_advisory_xact_lock(hashtextextended(p_beamter_id::text || ':' || (tagesbeginn::date)::text, 0));

      -- Feste Reihenfolge nach Erstellungszeitpunkt (dann id als Tie-Break):
      -- nur STRIKT FRÜHER erstellte, nicht abgelehnte Meldungen desselben
      -- Beamten zählen als "bereits verwendet" - das macht die Aufteilung
      -- unabhängig von der Anzahl gleichzeitig betroffener Meldungen (anders
      -- als eine einfache Summe über "alle anderen", die ab drei Meldungen
      -- an einem Tag die Gesamtstunden falsch verteilt hätte).
...
BEGIN
  -- Immer neu berechnen (nicht mehr nur bei geändertem Zeitraum) - eine
  -- reine Inhalts-Änderung (z. B. nur der Grund) darf vom Client
  -- mitgeschickte std_*-Werte nicht unverändert übernehmen. Für eine reine
  -- Genehmiger-Entscheidung reproduziert die Neuberechnung i. d. R. dieselben
  -- Werte (der Zeitraum bleibt ja gleich); weichen sie doch ab (weil sich
  -- inzwischen andere Meldungen desselben Tages geändert haben), ist das
  -- gewünscht - siehe enforce_ueberstunden_update unten.
...
-- Der Genehmiger darf über eine Meldung entscheiden, auch wenn deren
-- Aufschlüsselung sich durch die Neuberechnung gerade verschoben hat (siehe
-- oben) - std_* ist jetzt IMMER serverseitig berechnet, eine Abweichung von
-- OLD ist kein Hinweis auf eine unzulässige Änderung mehr und wird nicht
-- länger blockiert.
...
BEGIN
  IF current_setting('ueberstunden.recompute_cascade', true) = 'on' THEN
    -- Interner Neuberechnungs-Durchlauf (siehe
    -- ueberstunden_reallocate_nachfolgende) - ändert ausschließlich std_*
    -- und updated_at nachfolgender Meldungen desselben Beamten, keine
    -- Berechtigungsprüfung nötig (RLS greift bereits: nur die eigenen bzw.
    -- als Genehmiger alle Zeilen sind überhaupt sichtbar/änderbar).
    RETURN NEW;
  END IF;
...
-- Stößt bei allen (created_at,id) NACH der übergebenen Meldung liegenden,
-- nicht abgelehnten Meldungen desselben Beamten am angegebenen Tag eine
-- Neuberechnung an - ein reines "Berühren" der Zeile reicht, der
-- BEFORE-Trigger oben berechnet std_* dabei automatisch neu. Nur SPÄTER
-- erstellte Meldungen können betroffen sein (siehe Reihenfolge-Regel oben),
-- eine frühere Meldung hängt nie von einer später erstellten ab.
...
BEGIN
  ...
  -- Betrifft ausschließlich echte inhaltliche Änderungen (Zeitraum/Status) -
  -- der reine "Anstupser" aus ueberstunden_reallocate_nachfolgende
  -- (nur updated_at) löst hier bewusst NICHTS erneut aus, sonst würde jede
  -- Reallokation eine weitere nach sich ziehen.
