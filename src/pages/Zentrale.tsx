...
import { AlertTriangle, BellRing, BookOpen, BriefcaseBusiness, CheckCircle2, ClipboardList, Contact, FileClock, KeyRound, LayoutDashboard, MapPin, Pencil, Plus, Radio, Search, ShieldAlert, Trash2, UserRoundCheck, UsersRound, X } from 'lucide-react'
import { Navigate } from 'react-router-dom'
import MailDeliveries from '../components/MailDeliveries'
import { useAuth } from '../contexts/AuthContext'
import { logAudit } from '../lib/audit'
import { supabase } from '../lib/supabase'
import type { DutyAssignment, DutyFunctionConfig, DutyShift, IncidentDisposition, IncidentReport, KontrollauftragZielfunktion, OperationalPersonNote, OperationalPersonNoteCategory, ZentraleEntry, ZentraleEntryCategory, ZentraleEntryPriority, ZentraleEntryStatus } from '../lib/types'
...
export default function Zentrale() {
  ...
  const [entry, setEntry] = useState({ title: '', description: '', priority: 'normal' as ZentraleEntryPriority, status: 'offen' as ZentraleEntryStatus, validFrom: '', validUntil: '', location: '', responsible: '', reference: '', restricted: false, targetFunction: 'beide' as KontrollauftragZielfunktion })
  ...
  function openNewEntry() {
    ...
    setEditing(null); setEntry({ title: '', description: '', priority: 'normal', status: 'offen', validFrom: '', validUntil: '', location: '', responsible: '', reference: '', restricted: false, targetFunction: 'beide' }); setActiveTab(fallback); setShowEntryForm(true); setError('')
  }
  function openEdit(item: ZentraleEntry) { setEditing(item); setEntry({ title: item.title, description: item.description ?? '', priority: item.priority, status: item.status, validFrom: todayValue(item.valid_from), validUntil: todayValue(item.valid_until), location: item.location ?? '', responsible: item.responsible ?? '', reference: item.reference ?? '', restricted: item.restricted, targetFunction: item.target_function ?? 'beide' }); setShowEntryForm(true); setError('') }
  ...
  return <div>
    ...
    {!loading && !['uebersicht', 'einsaetze', 'personenhinweise', 'strassenzustand', 'brief'].includes(activeTab) ? <EntryList currentTab={currentTab} entries={visibleEntries} canManage={activeTab === 'kontrollauftrag' ? isGenehmiger : canManage} openNew={openNewEntry} openEdit={openEdit} /> : null}
    ...
    {showEntryForm ? <EntryModal entry={entry} setEntry={setEntry} editing={editing} category={(editing?.category ?? activeTab) as ZentraleEntryCategory} saving={saving} error={error} close={() => setShowEntryForm(false)} save={saveEntry} remove={deleteEntry} /> : null}
  </div>
}
