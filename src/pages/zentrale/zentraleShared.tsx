...
import { AV_BV_ART_LABEL, DISPOSITION_LABEL, FAHNDUNG_ART_LABEL, PERSON_NOTE_LABEL, composeIncidentLocation, formatTime, type BaustelleFormState, type IncidentFormState } from '../../lib/zentraleShared'
import type { DutyAssignment, DutyFunctionConfig, DutyShift, IncidentDisposition, IncidentReport, OperationalPersonNote, ZentraleAvBv, ZentraleBaustelle, ZentraleEntry, ZentraleFahndung } from '../../lib/types'
...
export function IncidentCards({ visibleIncidents, lageByIncidentId, canManage, canOperateZentrale, openLageForIncident, completeIncident, deleteIncident }: {
...
export function IncidentModal({ incident, setIncident, vdAvailable, contextEntries, contextPersonNotes, contextAvBv, contextFahndungen, priorIncidents, saving, error, locating, locateError, locate, close, save }: { incident: IncidentFormState; setIncident: Dispatch<SetStateAction<IncidentFormState>>; vdAvailable: boolean; contextEntries: ZentraleEntry[]; contextPersonNotes: OperationalPersonNote[]; contextAvBv: ZentraleAvBv[]; contextFahndungen: ZentraleFahndung[]; priorIncidents: IncidentReport[]; saving: boolean; error: string; locating: boolean; locateError: string; locate: () => Promise<void>; close: () => void; save: () => Promise<void> }) {
...
export function BaustellenList({ items, canManage, onConfirm, onEdit, onClose, onDelete }: { items: ZentraleBaustelle[]; canManage: boolean; onConfirm: (item: ZentraleBaustelle) => Promise<void>; onEdit: (item: ZentraleBaustelle) => void; onClose: (item: ZentraleBaustelle) => Promise<void>; onDelete: (item: ZentraleBaustelle) => Promise<void> }) {
