...
import { findSimilarObjects, objectLabel, personDisplayName, personLabel, useObjects, usePersons } from '../../lib/register'
...
type FormState = {
  ...
  ausnahmen: string
  hinweise: string
  /** Legt beim Anlegen fest, ob daraus ein Kontrollauftrag für die Streife entstehen soll - bewusste Entscheidung statt Automatik. */
  kontrolleErforderlich: boolean
}
...
export default function ZentraleAvBvPage() {
  ...
  async function locateArea(area: AreaForm) {
    ...
    const result = await geocodeLocation(area.label, null)
    setLocatingKey(null)
  ...
  return <div className="space-y-5">
    ...
    {showForm ? <Modal title={editing ? 'Schutzmaßnahme bearbeiten' : 'Schutzmaßnahme anlegen'} close={() => setShowForm(false)}>
      ...
      <PersonNameAutocomplete persons={persons} value={form.gefaehrderId || null} onChange={id => { setForm(current => ({ ...current, gefaehrderId: id ?? '' })); if (id) void loadHinweiseFor([id]) }} createdBy={profile?.id ?? null} onCreated={person => setPersons(current => [...current.filter(row => row.id !== person.id), person].sort((a, b) => personDisplayName(a).localeCompare(personDisplayName(b), 'de-AT')))} label="Gefährder" required includeBirthDate />
      ...
      <fieldset><legend className="text-xs font-medium text-gray-600">Geschützte Person(en) *</legend>
        {form.geschuetzteIds.length > 0 ? <ul className="mt-1 space-y-1">{form.geschuetzteIds.map(id => { const person = persons.find(row => row.id === id); return <li key={id} className="flex items-center justify-between gap-2 rounded-lg border border-gray-200 bg-gray-50 px-3 py-1.5 text-sm"><span>{person ? personLabel(person) : '—'}</span><button type="button" onClick={() => setForm(current => ({ ...current, geschuetzteIds: current.geschuetzteIds.filter(pid => pid !== id) }))} aria-label="Person entfernen" className="text-gray-400 hover:text-red-600"><X className="h-4 w-4" /></button></li> })}</ul> : null}
        <div className="mt-2"><PersonNameAutocomplete key={form.geschuetzteIds.length} persons={persons.filter(person => person.id !== form.gefaehrderId && !form.geschuetzteIds.includes(person.id))} value={null} onChange={id => { if (id) { setForm(current => ({ ...current, geschuetzteIds: current.geschuetzteIds.includes(id) ? current.geschuetzteIds : [...current.geschuetzteIds, id] })); void loadHinweiseFor([id]) } }} createdBy={profile?.id ?? null} onCreated={person => setPersons(current => [...current.filter(row => row.id !== person.id), person].sort((a, b) => personDisplayName(a).localeCompare(personDisplayName(b), 'de-AT')))} label="Weitere geschützte Person hinzufügen" includeBirthDate /></div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4"><Field label="PAD-Aktenzahl *" value={form.pad} onChange={value => setForm(current => ({ ...current, pad: value }))} /><Field label="Ausstellende Stelle" value={form.stelle} onChange={value => setForm(current => ({ ...current, stelle: value }))} /><Field label="Beginn / Ausspruch *" type="datetime-local" value={form.beginn} onChange={value => setForm(current => ({ ...current, beginn: value, ende: current.massnahme === 'bv_av' ? defaultBvAvEnd(value) : current.ende }))} />{form.massnahme === 'bv_av'
        ? <div><p className="text-xs font-medium text-gray-600">Reguläres Ende</p><div className="mt-1 rounded-lg border border-gray-200 bg-gray-50 px-3 py-2.5 text-sm text-gray-800">{form.ende ? new Date(form.ende).toLocaleString('de-AT') : '—'}</div><p className="mt-1 text-[11px] text-gray-500">Automatisch: 14 Tage ab Ausspruch. Eine vorzeitige Aufhebung durch die Sicherheitsbehörde wird separat erfasst.</p></div>
        : <Field label="Ende *" type="datetime-local" value={form.ende} onChange={value => setForm(current => ({ ...current, ende: value }))} />}</div>
      ...
      <div className="space-y-3"><div className="flex items-center justify-between"><h3 className="text-sm font-semibold text-gray-900">{form.massnahme === 'bv_av' ? 'Wohnungs-Schutzbereich' : 'Gerichtliche Schutzbereiche'}</h3>{form.massnahme === 'ev' ? <button type="button" onClick={() => setAreas(current => [...current, newArea()])} className="text-sm font-semibold text-blue-700"><Plus className="mr-1 inline h-4 w-4" />Weiterer Ort</button> : null}</div>{areas.map((area, index) => <div key={area.key} className="rounded-xl border border-gray-200 p-3 space-y-3">
        ...
        <ObjectAddressAutocomplete objects={objects} value={area.objectId || null} onChange={objectId => selectObject(area.key, objectId ?? '')} createdBy={profile?.id ?? null} onCreated={created => setObjects(current => [...current, created].sort((a, b) => a.address.localeCompare(b.address, 'de-AT')))} label="Objekt aus Stammdaten (optional)" />
        <Field label="Adresse / Bezeichnung * (mit PLZ und Ort, falls außerhalb Dornbirn)" value={area.label} onChange={value => updateArea(area.key, { label: value, confirmed: false })} />
      ...
      <details className="rounded-xl border border-gray-200 p-3"><summary className="cursor-pointer text-sm font-semibold text-gray-700">Weitere Angaben (nur wenn vorhanden)</summary><div className="mt-4 space-y-4"><Field label="Externe / gerichtliche Aktenzahl" value={form.externeAkte} onChange={value => setForm(current => ({ ...current, externeAkte: value }))} />{form.massnahme === 'ev' ? <label className="block text-xs font-medium text-gray-600">Status<select className={inputClass} value={form.status} onChange={event => setForm(current => ({ ...current, status: event.target.value as SchutzfallStatus }))}><option value="aktiv">Aktiv</option><option value="aufgehoben">Aufgehoben</option><option value="abgelaufen">Abgelaufen</option></select></label> : <div className="rounded-lg border border-gray-200 bg-gray-50 px-3 py-2 text-sm text-gray-700">Status: {form.status === 'aufgehoben' ? 'Durch Sicherheitsbehörde aufgehoben' : form.status === 'abgelaufen' ? 'Abgelaufen' : 'Aktiv'}</div>}<label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={form.waffenverbot} onChange={event => setForm(current => ({ ...current, waffenverbot: event.target.checked }))} />Vorläufiges Waffenverbot beachten</label><label className="block text-xs font-medium text-gray-600">Schlüsselstatus<select className={inputClass} value={form.schluesselStatus} onChange={event => setForm(current => ({ ...current, schluesselStatus: event.target.value as Schutzfall['schluessel_status'] }))}><option value="nicht_erfasst">Nicht erfasst</option><option value="abgenommen">Abgenommen</option><option value="verwahrt">Verwahrt</option><option value="gericht">Bei Gericht</option><option value="ausgefolgt">Ausgefolgt</option></select></label>{form.schluesselStatus !== 'nicht_erfasst' ? <Field label="Verwahrort / Übergabe" value={form.verwahrort} onChange={value => setForm(current => ({ ...current, verwahrort: value }))} /> : null}<Area label="Ausnahmen" value={form.ausnahmen} onChange={value => setForm(current => ({ ...current, ausnahmen: value }))} /><Area label="Einsatzrelevante Hinweise" value={form.hinweise} onChange={value => setForm(current => ({ ...current, hinweise: value }))} /></div></details>
      {error ? <ErrorMessage text={error} /> : null}<div className="flex flex-wrap justify-end gap-3 pt-2">{editing && canOperate && editing.massnahme === 'bv_av' && editing.status === 'aktiv' ? <button type="button" disabled={saving} onClick={async () => {
        if (!window.confirm('BV/AV wirklich als durch die Sicherheitsbehörde aufgehoben markieren?')) return
        setSaving(true); setError('')
        const result = await supabase.from('schutzfaelle').update({
          status: 'aufgehoben',
