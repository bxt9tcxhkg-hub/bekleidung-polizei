...
export default function ZentraleAvBvPage() {
  ...
  return <div className="space-y-5">
    ...
    {showForm ? <Modal title={editing ? 'Schutzmaßnahme bearbeiten' : 'Schutzmaßnahme anlegen'} close={() => setShowForm(false)}>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4"><Field label="PAD-Aktenzahl *" value={form.pad} onChange={value => setForm(current => ({ ...current, pad: value }))} /><Field label="Ausstellende Stelle" value={form.stelle} onChange={value => setForm(current => ({ ...current, stelle: value }))} /><Field label="Beginn *" type="datetime-local" value={form.beginn} onChange={value => setForm(current => ({ ...current, beginn: value, ende: current.massnahme === 'bv_av' ? defaultBvAvEnd(value) : current.ende }))} />{form.massnahme === 'bv_av'
