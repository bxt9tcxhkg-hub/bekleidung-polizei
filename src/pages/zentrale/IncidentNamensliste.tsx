...
import type { EreignisDimension, NamenslistePerson } from '../../lib/types'
...
export default function IncidentNamensliste({ incidentId, incidentTitel, canOperate = true, mode = 'feld', ereignisstufe, onChanged }: { incidentId: string; incidentTitel: string; canOperate?: boolean; mode?: 'zentrale' | 'feld'; ereignisstufe?: EreignisDimension; onChanged?: () => void }) {
  ...
  useEffect(() => {
    if (SECONDARY_LISTS.includes(listenart) && !erlaubteSekundaerlisten.includes(listenart)) setListenart('haus')
  }, [erlaubteSekundaerlisten, listenart])
  ...
  const hatZweiDruckvarianten = listenart === 'evakuierung' || listenart === 'kontrolle' || listenart === 'befragung' || listenart === 'unterbringung'
  ...
  return <div className="space-y-3">
    ...
    <div className="flex flex-wrap items-center gap-2">
      <button type="button" onClick={() => drucken(false)} disabled={personen.length === 0} className="inline-flex items-center gap-1.5 text-xs font-semibold text-blue-800 border border-blue-200 px-2 py-1.5 rounded-md disabled:opacity-40 disabled:cursor-not-allowed"><Printer className="w-3.5 h-3.5" /> {hatZweiDruckvarianten ? 'PDF (Stand)' : 'PDF'}</button>
      {hatZweiDruckvarianten ? <button type="button" onClick={() => drucken(true)} disabled={personen.length === 0} className="inline-flex items-center gap-1.5 text-xs font-semibold text-gray-700 border border-gray-300 px-2 py-1.5 rounded-md disabled:opacity-40 disabled:cursor-not-allowed"><Printer className="w-3.5 h-3.5" /> PDF (leer zum Erfassen)</button> : null}
      ...
      >
        ...
        {erlaubteSekundaerlisten.map(key => <option key={key} value={key}>{LISTENART_LABEL[key]} ({counts[key]})</option>)}
      </select> : null}
    ...
    {isZentrale && canOperate && listenart === 'haus' && personen.length > 0 ? <div className="rounded-xl border border-gray-200 bg-gray-50 p-3">
      ...
      <div className="mt-2 flex flex-wrap gap-2">
        ...
        {erlaubteSekundaerlisten.includes('kontrolle') ? <button type="button" disabled={busy} onClick={() => void listeBereitstellen('kontrolle')} className="rounded-lg border border-blue-200 bg-white px-2.5 py-1.5 text-xs font-semibold text-blue-800 disabled:opacity-50">
          Kontrollliste bereitstellen
        </button> : null}
        {erlaubteSekundaerlisten.includes('befragung') ? <button type="button" disabled={busy} onClick={() => void listeBereitstellen('befragung')} className="rounded-lg border border-blue-200 bg-white px-2.5 py-1.5 text-xs font-semibold text-blue-800 disabled:opacity-50">
