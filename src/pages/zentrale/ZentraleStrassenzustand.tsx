...
export default function ZentraleStrassenzustand() {
  ...
  return <div>
    ...
    {showForm ? <Modal wide title={editingBericht ? `Bericht Nr. ${editingBericht.nummer} bearbeiten` : 'Straßenzustandsbericht erfassen'} close={closeForm}>
      <div className="grid grid-cols-1 lg:grid-cols-[minmax(280px,1fr)_minmax(320px,0.95fr)] gap-4 items-start">
      <div className="space-y-4">
        {rows.map((row, index) => <div key={index} className="rounded-xl border border-gray-200 p-3 space-y-3">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            ...
            {!row.strasseId ? <div className="sm:col-span-2"><SonstigeStrasseOrt name={row.strasseFreitext} onName={value => patchRow(index, { strasseFreitext: value })} lat={row.pinLat ?? null} lng={row.pinLng ?? null} onPoint={(lat, lng, label) => patchRow(index, { strasseFreitext: label, pinLat: lat, pinLng: lng })} /></div> : <div className="sm:col-span-2 text-xs text-gray-500">Lage der Stammdaten-Straße erscheint rechts auf der Karte.</div>}
      ...
      <LeafletMap
        height={560}
        markers={rows.filter(row => row.pinLat != null && row.pinLng != null).map(row => ({ lat: row.pinLat as number, lng: row.pinLng as number, popup: row.strasseFreitext || 'Sonstige Straße' }))}
        lines={rows.flatMap(row => {
          const strasse = strassen.find(item => item.id === row.strasseId)
          if (!strasse || strasse.start_lat == null || strasse.start_lng == null || strasse.end_lat == null || strasse.end_lng == null) return []
          return [{ points: (strasse.path && strasse.path.length >= 2 ? strasse.path : [[strasse.start_lat, strasse.start_lng], [strasse.end_lat, strasse.end_lng]]) as readonly [number, number][], popup: strasse.name, color: '#ea580c' }]
        })}
        onMapClick={(lat, lng) => {
          const index = rows.findIndex(row => !row.strasseId)
          if (index < 0) return
          void reverseGeocode(lat, lng).then(result => {
            const label = [result?.street, result?.houseNumber].filter(Boolean).join(' ') || result?.displayName || `${lat.toFixed(5)}, ${lng.toFixed(5)}`
            patchRow(index, { strasseFreitext: label, pinLat: lat, pinLng: lng })
          })
        }}
      />
      </div>
