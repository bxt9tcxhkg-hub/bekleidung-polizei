...
export default function FleetVehicle() {
  ...
  const [operationalStatus, setOperationalStatus] = useState<'verfuegbar' | 'werkstatt' | 'ausser_dienst'>('verfuegbar')
  ...
  function openEdit() {
    ...
    setOperationalStatus(vehicle.operational_status ?? 'verfuegbar')
    setOperationalStatusNote(vehicle.operational_status_note ?? '')
  ...
  async function saveVehicle() {
    ...
    const { error: updateError } = await supabase.from('fleet_vehicles').update({
      ...
      operational_status_note: operationalStatusNote.trim() || null,
    }).eq('id', vehicle.id)
    setSaving(false)
  ...
  return <div>
    ...
    <div className="flex items-center justify-between gap-3 mb-4"><div className="flex items-center gap-3 min-w-0"><div className="bg-blue-50 text-blue-700 p-2.5 rounded-xl flex-shrink-0"><VehicleIcon className="w-5 h-5" /></div><div className="min-w-0"><p className="text-xs font-semibold uppercase tracking-wide text-gray-500">{vehicle.kind}</p><h1 className="text-xl font-bold text-gray-900 truncate">{vehicle.name}{vehicle.call_sign ? <span className="text-gray-400 font-normal"> · {vehicle.call_sign}</span> : null}</h1><div className="mt-1 flex flex-wrap items-center gap-2"><span className={`text-xs font-semibold px-2 py-0.5 rounded-full ${vehicle.operational_status === 'verfuegbar' ? 'bg-green-100 text-green-800' : vehicle.operational_status === 'werkstatt' ? 'bg-amber-100 text-amber-800' : 'bg-red-100 text-red-800'}`}>{vehicle.operational_status === 'verfuegbar' ? 'Verfügbar' : vehicle.operational_status === 'werkstatt' ? 'In Werkstatt' : 'Außer Dienst'}</span>{vehicle.operational_status_note ? <span className="text-xs text-gray-500">{vehicle.operational_status_note}</span> : null}</div></div></div>{canManage ? <button type="button" onClick={openEdit} className="inline-flex items-center gap-2 border border-gray-300 text-gray-700 text-sm font-medium px-3 py-2 rounded-lg hover:bg-gray-50 flex-shrink-0"><Pencil className="w-4 h-4" /> Bearbeiten</button> : null}</div>
