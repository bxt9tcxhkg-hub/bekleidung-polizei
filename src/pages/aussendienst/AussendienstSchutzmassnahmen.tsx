...
import { assessDistance, firstControlDeadline, hasInitialControl, haversineMeters, MASSNAHME_LABEL, SCHUTZ_SELECT, type Schutzfall } from '../../lib/schutzmassnahmen'
...
export default function AussendienstSchutzmassnahmen() {
  ...
  const load = useCallback(async () => {
    ...
    const [result, orderResult] = await Promise.all([
      ...
      supabase.from('zentrale_entries').select('schutzfall_id').eq('category', 'kontrollauftrag').neq('status', 'erledigt').not('schutzfall_id', 'is', null),
    ])
    setLoading(false)
    ...
    setItems(rows)
    setKontrolliert(new Set((orderResult.data ?? []).map(row => row.schutzfall_id as string)))
  ...
  async function saveControl(outcome: 'Ohne relevante Feststellung' | 'PAD-Dokumentation erforderlich') {
    if (!selected || !profile?.id) return
    setSaving(true); setError('')
    const result = await supabase.from('schutzkontrollen').insert({ schutzfall_id: selected.id, kontrolliert_am: new Date().toISOString(), notiz: outcome, created_by: profile.id })
    if (result.error) { setSaving(false); setError('Die Kontrolle konnte nicht gespeichert werden.'); return }
    const completed = await supabase.from('zentrale_entries').update({ status: 'erledigt', erledigt_at: new Date().toISOString() }).eq('category', 'kontrollauftrag').eq('schutzfall_id', selected.id).neq('status', 'erledigt').select('id')
    setSaving(false)
    if (completed.error || !completed.data?.length) { setError('Die Kontrolle wurde erfasst, der Kontrollauftrag konnte aber nicht abgeschlossen werden.'); return }
    logAudit('Schutzmaßnahme kontrolliert', `${MASSNAHME_LABEL[selected.massnahme]} · PAD ${selected.pad_aktenzahl}`)
    setNotice('Organisatorischer Kontrollstatus gespeichert. Feststellungen und Maßnahmen werden ausschließlich im PAD dokumentiert.'); await load()
  }

  return <div className="space-y-5">
    <div><p className="text-xs font-bold uppercase tracking-wider text-blue-700">Außendienst</p><h2 className="text-2xl font-bold text-gray-900 mt-1">Schutzmaßnahmen</h2><p className="text-sm text-gray-500 mt-1">Einsatzinformation und organisatorischer Kontrollauftrag. Feststellungen und Maßnahmen gehören ausschließlich ins PAD.</p></div>
    ...
    {loading ? <div className="py-8 text-center text-sm text-gray-500">Schutzmaßnahmen werden geladen…</div> : items.length === 0 ? <div className="rounded-2xl border border-dashed border-gray-300 bg-white p-8 text-center text-sm text-gray-500">Keine aktiven Schutzmaßnahmen.</div> : <div className="grid gap-3">{items.map(item => {
      ...
      const overdue = kontrolliert.has(item.id) && !initialDone && firstControlDeadline(item).getTime() < now
      return <button type="button" key={item.id} onClick={() => { setSelected(item); setOwnPoint(null); setDangerPoint(null); setProtectedPoint(null) }} className="rounded-2xl border border-gray-200 bg-white p-4 text-left hover:border-blue-300">
    ...
    {selected ? <section className="rounded-2xl border border-blue-200 bg-white p-4 sm:p-5 space-y-4">
      ...
      {selected.massnahme === 'bv_av' ? <div className="rounded-xl border border-gray-200 p-3">
        ...
        {personDistance !== null ? <div className={`mt-3 rounded-lg border p-3 text-sm ${assessDistance(personDistance, 100, (dangerPoint?.accuracy ?? 0) + (protectedPoint?.accuracy ?? 0)).color}`}><strong>Luftlinie ca. {Math.round(personDistance)} m</strong><p className="mt-1 text-xs">Nur Orientierung; wird nicht als Feststellung gespeichert.</p></div> : null}
      </div> : null}
      {kontrolliert.has(selected.id) ? <div className="rounded-xl border border-blue-200 bg-blue-50 p-3">
        <p className="text-sm font-semibold text-blue-900">Organisatorischen Kontrollauftrag abschließen</p>
        <p className="mt-1 text-xs text-blue-800">Datum, Uhrzeit und Streife werden automatisch erfasst. Keine freie Einsatzdokumentation im Portal.</p>
        <div className="mt-3 grid gap-2 sm:grid-cols-2">
          <button type="button" onClick={() => void saveControl('Ohne relevante Feststellung')} disabled={saving} className="inline-flex min-h-12 items-center justify-center gap-2 rounded-xl bg-green-700 px-4 py-3 text-sm font-semibold text-white disabled:opacity-60"><CheckCircle2 className="h-5 w-5" />Ohne relevante Feststellung</button>
          <button type="button" onClick={() => void saveControl('PAD-Dokumentation erforderlich')} disabled={saving} className="min-h-12 rounded-xl border border-amber-300 bg-amber-50 px-4 py-3 text-sm font-semibold text-amber-900 disabled:opacity-60">PAD-Dokumentation erforderlich</button>
        </div>
      </div> : <p className="rounded-xl bg-gray-50 p-3 text-sm text-gray-600">Diese Schutzmaßnahme dient ausschließlich der Einsatzinformation. Es besteht kein Kontrollauftrag.</p>}
    </section> : null}
  </div>
}
