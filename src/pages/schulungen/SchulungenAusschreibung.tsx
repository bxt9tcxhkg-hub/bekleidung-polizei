...
export default function SchulungenAusschreibungPanel({ canManage, isGenehmiger }: { canManage: boolean; isGenehmiger: boolean }) {
  ...
  async function save() {
    ...
    setSaving(true)
    setError('')
    const { error: insertError } = await supabase.from('schulungen_sessions').insert({ ...result.payload, created_by: profile?.id ?? null })
    if (insertError) {
    ...
    logAudit('Schulung ausgeschrieben', `${module?.name ?? moduleId} ${result.payload.session_date}`)
    setShowForm(false)
    setSaving(false)
    try {
      await load()
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg">
            <div className="flex items-center justify-between px-6 py-4 border-b">
              <h2 className="font-bold text-gray-900">Schulung ausschreiben</h2>
