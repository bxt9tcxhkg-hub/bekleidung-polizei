  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        ...
        <button onClick={() => { setForm({ user_id: '', amount: '', refund_date: new Date().toISOString().split('T')[0], note: '' }); setUserSearch(''); setError(''); setShowForm(true) }} className="flex items-center gap-2 bg-blue-800 hover:bg-blue-900 text-white text-sm font-medium px-3 py-2.5 sm:px-4 rounded-lg transition-colors flex-shrink-0" title="Neue Erstattung">
          <Plus className="w-4 h-4 flex-shrink-0" /><span className="hidden sm:inline">Neue Erstattung</span>
        </button>
      </div>
