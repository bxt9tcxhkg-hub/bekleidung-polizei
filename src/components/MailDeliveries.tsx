 ...
 * nach Person (aus dem zentralen Personen-Register) gruppiert, mit Schnellaktionen.
 * Der Akteneigentümer ist automatisch, wer den Eintrag erfasst hat. Ablauf:
 * Feld-Aktion setzen (verschwindet aus der offenen Liste, bleibt für den
 * Eigentümer als Rückmeldung sichtbar) → der Eigentümer schließt den Akt
 * endgültig, danach ist er ganz weg. Unabhängig vom heutigen Dienst nutzbar —
 * kein Erfordernis, "im operativen Dienst" zu sein.
 */
export default function MailDeliveries({ onlyOpen = false }: { onlyOpen?: boolean }) {
  ...
  const canManage = isStrictAdmin || isGenehmiger || (operativeModeActive && (areaRoles?.find(row => row.area === 'zentrale')?.roles ?? []).some(role => ['sachbearbeiter', 'admin'].includes(role)))
  ...
  async function save() {
    ...
    const { error: insertError } = await supabase.from('mail_deliveries').insert({
      person_id: form.personId, kind: form.kind,
