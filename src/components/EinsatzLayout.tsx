...
const baseItems = [
  ...
  { to: '/einsatz/einsatzmittel', label: 'Einsatzmittel', icon: Shield },
  { to: '/einsatz/training', label: 'Einsatztraining', icon: Target },
  { to: '/einsatz/unterlagen', label: 'Unterlagen', icon: BookOpen },
]
      ...
      <aside className={`fixed lg:static inset-y-0 left-0 z-30 w-60 bg-blue-950 flex flex-col transition-transform duration-200 ${
        ...
        <div className="flex items-center gap-3 px-4 py-5 border-b border-blue-900">
          ...
          <div className="min-w-0">
            ...
            <p className="text-blue-300 text-xs">Einsatzmittel & Training</p>
          </div>
        </div>
            ...
            {baseItems.map(({ to, label, icon: Icon }) => {
              ...
              return (
                ...
                >
                  ...
                  {canManage && label === 'Einsatzmittel' ? (
                    <ClipboardCheck className="w-3.5 h-3.5 opacity-70" />
                  ) : null}
                </Link>
              )
            })}
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <header className="lg:hidden flex items-center gap-3 px-4 py-3 bg-white border-b border-gray-200">
          ...
          <div className="flex items-center gap-2 min-w-0">
            ...
            <span className="font-semibold text-gray-900 text-sm truncate">Einsatzmittel & Training</span>
          </div>
          {sidebarOpen ? (
