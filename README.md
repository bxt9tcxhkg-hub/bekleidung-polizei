...
| `20260926120000_innendienst_kassa_schichtweit_sichtbar.sql` | `innendienst_shift_tasks`-Lese-Policy additiv erweitert: zusätzlich zur eigenen Zeile auch fremde Zeilen lesbar, für die laut `duty_assignments` eine Innendienst-Diensteinteilung am aktuellen Diensttag (`operational_today()`) exakt auf dasselbe `duty_date`+`shift` existiert (korreliertes `EXISTS`, bewusst enger als `is_operative_duty_today()`, das ohne Bezug zur Zeile/zum Tag jede Tagesfunktion an jedem beliebigen - auch vergangenen - Tag durchließe) - bei einer Schichtübernahme war die bereits erfolgte Kassenabrechnung der/des Vorgängerin/Vorgängers zuvor unter RLS unsichtbar, weil nur die eigene Zeile lesbar war. Client fragt jetzt alle Schichtaufgaben des laufenden Diensttags (`operationalToday()` statt `todayLocal()`, wegen über Mitternacht laufendem Nachtdienst) statt nur der eigenen ab; Kassenabrechnungs-Karte in der Innendienst-Übersicht zeigt einen Hinweis, wenn eine andere Person die Kasse für dieselbe Schicht schon bestätigt hat |
