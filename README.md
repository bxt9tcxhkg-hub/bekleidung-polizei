...
| `20260919030000_sole_genehmiger_name.sql` | RPC `sole_genehmiger_name()` (SECURITY DEFINER): liefert den Namen des einzigen aktiven Admin/Genehmiger/Approver, sonst `null` (mehrdeutig bei mehreren). Für den Ausdruck einer Überstundenmeldung vor der Entscheidung (`Ueberstunden.tsx::printMeldung`) - ohne diese Funktion stand dort immer "–" statt des voraussichtlichen Unterzeichners, und ein normaler Bediensteter darf die Rollen anderer Profile laut RLS gar nicht selbst abfragen |
