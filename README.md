...
| `20260925162958_dienstplan_freiplanungswuensche_praezisierung.sql` | Präzisiert die Freiplanungswünsche nach Rückmeldung des Kommandanten: `frei`-Typ ersetzt durch `frei_tag`/`frei_nacht` (ein ganzer freier Tag braucht beide) + neuer Typ `urlaub` (ganztägig, aber nur 1 Kontingenteinheit); pro Person/Tag jetzt mehrere Wunsch-Zeilen möglich (Unique-Constraint erweitert um `wunsch`); `dienstplan_wunsch_loeschen` bekommt zusätzlich `p_wunsch`. `dienstplan_person_einstellungen.beschaeftigungsgrad` läuft jetzt auf der hausinternen Skala mit 111 = Vollzeit (Check-Constraint + Default angepasst) - wirkt sich auf die Sollstunden-Formel UND das monatliche Freiplanungswunsch-Kontingent (18 bei Vollzeit, siehe `lib/dienstplanWunsch.ts`) aus. Kontingent und "max. 6 am Stück"-Regel werden bewusst nur clientseitig geprüft |
| `20260925164438_dienstplan_wunsch_praeferenzen_entfernen.sql` | Entfernt die unverbindlichen Präferenzen "Tagdienst/Nachtdienst bevorzugt" aus `dienstplan_wuensche` wieder - es bleiben nur die drei kontingentierten Freiplanungswünsche `frei_tag`/`frei_nacht`/`urlaub` |
...
| `20260926074234_zentrale_notizen.sql` | Neue Tabelle `zentrale_notizen` für das schwebende Notiz-Widget in Zentrale/Innendienst (`NotizenWidget.tsx`): Notizen mit `bereich` (`zentrale`/`innendienst`) und `sichtbarkeit` (`privat`/`geteilt`). Lesen folgt dem Muster aus `20260919070000_tagesfunktion_zugriff.sql` (`has_portal_area_access('zentrale') or is_operative_duty_today()`), Schreiben/Ändern/Löschen bewusst enger auf `is_zentralist_on_duty()` beschränkt; private Notizen bleiben zusätzlich immer für die eigene `autor_id` sichtbar/bearbeitbar |
| `20260926120000_innendienst_kassa_schichtweit_sichtbar.sql` | Neuer Helfer `is_innendienst_shift_duty(p_duty_date, p_shift)` (analog `has_portal_area_access('<bereich>')`, parametrisiert): true nur, wenn laut `duty_assignments` eine Innendienst-Diensteinteilung am aktuellen Diensttag (`operational_today()`) exakt auf das übergebene `duty_date`+`shift` existiert - bewusst enger als `is_operative_duty_today()`, das ohne Bezug zur Zeile/zum Tag jede Tagesfunktion an jedem beliebigen, auch vergangenen Tag durchließe. `innendienst_shift_tasks`-Lese-Policy additiv um diesen Helfer erweitert: zusätzlich zur eigenen Zeile jetzt auch fremde Zeilen der/des Vorgängerin/Vorgängers derselben Schicht lesbar (zuvor unter RLS unsichtbar, weil nur die eigene Zeile lesbar war). Client fragt jetzt alle Schichtaufgaben des laufenden Diensttags (`operationalToday()` statt `todayLocal()`, wegen über Mitternacht laufendem Nachtdienst) statt nur der eigenen ab; Kassenabrechnungs-Karte in der Innendienst-Übersicht zeigt einen Hinweis (nach `kasse_confirmed_at` absteigend sortiert, falls mehrere), wenn eine andere Person die Kasse für dieselbe Schicht schon bestätigt hat |
